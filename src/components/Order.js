...
const Orders = () => {
    ...
    return (
        <div className="container" style={{marginTop: "50px", paddingBottom: "50px"}}>
            <h1>My Orders</h1>
            { orders.length > 0 && <div className="accordion" id="orderItems">
                {
                                    ...
                                    <div className="card-header" id={"heading"+(index)}>
                                        <h2 className="mb-0">
                                            <button className="btn btn-link collapsed" type="button" data-toggle="collapse" data-target={"#collapse"+(index)} aria-expanded="true" aria-controls={"collapse"+(index)}>
                                                Order # {each.orderid} at {each.orderdate} by {each.name}
                                            </button>
                                        </h2>
                                    </div>

                                    <div id={"collapse"+(index)} className="collapse" aria-labelledby={"heading"+(index)}
                                         data-parent="#orderItems">
                                        {
            ...
            {orders.length === 0 && <div className="accordion">
                No Orders Found!!
            </div>}
        </div>
    )
